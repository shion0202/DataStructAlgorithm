...
RBNode* RBTree::Find(const int& value)
{
...
void RBTree::Clear()
{
	Clear_Implement(root);
	root = NIL;
}

RBNode* RBTree::Find_Implement(RBNode* current, const int& value)
{
...
void RBTree::Insert_Implement(RBNode* current, const int& value)
{
	if (value < current->data)
	{
		if (current->left == NIL)
...
void RBTree::Delete_Implement(const int& value)
{
	RBNode* current = root;
...
void RBTree::InOrder_Implement(RBNode* start)
{
	if (!start || start == NIL)
		return;
...
void RBTree::PrintTree_Implement(RBNode* start, string prefix, bool isRight)
{
	if (start && start != NIL)
	{
		cout << prefix;
...
void RBTree::Clear_Implement(RBNode* start)
{
	if (start == NIL)
	{
		return;
	...
	Clear_Implement(start->left);
	Clear_Implement(start->right);
	delete start;
}

void RBTree::RebalanceAfterInsert(RBNode* node)
{
	// 삽입 후 트리의 균형이 맞는지 확인
...
void RBTree::RebalanceAfterDelete(RBNode* node)
{
	RBNode* sibling;
...
RBNode* RBTree::GetSuccessor(RBNode* start)
{
...
void RBTree::Transplant(RBNode* current, RBNode* newNode)
{
